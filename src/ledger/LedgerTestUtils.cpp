...
namespace stellar
{
	namespace LedgerTestUtils
	{
		...
		static auto validAliasEntryGenerator = autocheck::map(
			[](AliasEntry&& ale, size_t s) {
			makeValid(ale);
			return ale;
		},
			autocheck::generator<AliasEntry>());
		...
		AliasEntry
			generateValidAliasEntry(size_t b)
		{
			return validAliasEntryGenerator(b);
		}

		std::vector<AliasEntry>
			generateValidAliasEntries(size_t n)
		{
			static auto vecgen = autocheck::list_of(validAliasEntryGenerator);
			return vecgen(n);
		}
